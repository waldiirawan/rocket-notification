...
class mailMessage {
  ...
  render(option = { inline: true }) {
    ...
    return new Promise((resolve) => {
      const html = view.render(this._markdownMessage, { slots: lines, env: process.env, subject: this._mailerMessage.subject, data: this._data })
      if (option.inline) {
  ...
  greetingView(item) {
    return view.render(`${this._baseMarkdown}.greeting`, {
      greeting: this.nl2br(item.value)
    })
  }

  lineView(item) {
    return view.render(`${this._baseMarkdown}.line`, {
      line: this.nl2br(item.value)
    })
  }

  customView(item) {
    return view.render(`${this._baseMarkdown}.${item.view}`, {
      line: this.nl2br(item.value)
    })
  }

  buttonView(item) {
    return view.render(`${this._baseMarkdown}.button`, {
      name: item.name,
      url: item.url,
      color: item.color
    })
